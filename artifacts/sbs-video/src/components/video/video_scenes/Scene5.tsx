    ...
    >
      ...
      <div className="relative w-full max-w-6xl h-[55vh] flex gap-6 px-8" style={{ perspective: 1200 }}>
        <motion.div
          ...
          animate={phase >= 1 ? { opacity: 1, rotateY: 0 } : { opacity: 0, rotateY: -15 }}
        ...
        <div className="w-[40%] flex flex-col gap-6">
              ...
              {[0, 1, 2].map((i) => (
                <div key={i} className="flex items-center justify-between p-3 rounded-lg bg-slate-800 border border-slate-700/50">
                  <div className="space-y-1">
                    ...
                    <div className="text-xs text-slate-500">£{(i + 1) * 180}.00</div>
                  </div>
          ...
          <motion.div
            className="h-20 bg-indigo-950 border border-indigo-500/30 rounded-2xl flex items-center justify-center gap-4 shadow-lg"
            initial={{ opacity: 0, y: 50 }}
            animate={phase >= 3 ? { opacity: 1, y: 0 } : { opacity: 0, y: 50 }}
            transition={{ type: 'spring' }}
          >
            <div className="w-10 h-10 rounded-full bg-white flex items-center justify-center font-bold text-indigo-600 shrink-0">X</div>
