  ...
  const staffQuotesHtml = staffQuotesClean.length === 0 ? "" :
    ...
    staffQuotesClean.map(q => {
      ...
      const avatar = q.imageUrl
        ? `<img src="${q.imageUrl}" alt="${q.name}" style="width:112px;height:112px;border-radius:50%;object-fit:cover;flex-shrink:0;border:3px solid #1e3a5f;box-shadow:0 2px 8px rgba(30,58,95,0.25)" />`
