...
import {
  ...
  customerEmployeesTable, customerDeliveryAddressesTable,
} from "@workspace/db";
...
router.get("/dispatch/orders", async (req, res): Promise<void> => {
  ...
  const result = orders
    .map((order) => {
      ...
      const hasAnyComplete = orderWs.some((w) => w.status === "complete");

      if (!hasAnyComplete) return null;

      const allComplete = orderWs.length > 0 && orderWs.every((w) => w.status === "complete");
